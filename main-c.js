/* Build Enola — Option C JS © Sean Gal | enolarevenu.com */
...
document.querySelectorAll('.reveal-card').forEach(card => {
  ...
  if (!cardGroups.has(p)) {
    ...
    new IntersectionObserver(([e]) => {
      if (e.isIntersecting) p.querySelectorAll('.reveal-card').forEach((el, i) => setTimeout(() => el.classList.add('is-visible'), i * 80));
...
if (sticky && hero) {
  new IntersectionObserver(([e]) => sticky.classList.toggle('is-visible', !e.isIntersecting), { threshold: 0 }).observe(hero);
}
